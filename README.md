...
Whenever a bump actually happens (auto-detected or explicit), a further play updates `roles/semaphore_provision/files/semaphore.container`'s `Image=` line in this repo to match, and commits + pushes it to `origin/main` directly — so a future fresh provision always starts at whatever version is currently running, with no manual repo sync step. This is the one place in this repo where automation pushes to GitHub on its own rather than the user doing it; it authenticates with a **`GITHUB_PUSH_TOKEN`** (a GitHub Personal Access Token with write access to this repo), injected the same way as every other secret here. The token briefly appears in the push command's argv while it runs (visible to `ps` on that host during that moment) — accepted as a reasonable tradeoff for a single-tenant homelab host, but worth knowing if that host's threat model ever changes.

**Postgres version checks (notify only, never auto-applied)**: `semaphore-db.container` is pinned to a floating major-version tag on purpose (see [Provisioning a new Semaphore peer](#provisioning-a-new-semaphore-peer)) — routine patches within that major version already apply automatically via the normal container update above, so there's nothing to auto-bump. `update-semaphore-peer.yml` ends with a read-only check, run in both directions, comparing the running Postgres major version against the latest major tag published on Docker Hub, and sends its own email the moment a newer major version exists — nothing is ever pulled or restarted as a result, since a major-version bump needs a real migration (`pg_upgrade` or dump/restore): Postgres refuses to start against a data directory from a different major version, so this can't be a simple image-swap-and-restart like Semaphore's own bump above.
