...
| `configure-bunkerweb.yml` | Deploys `bunkerweb101`'s global settings and per-service reverse-proxy config (one file per proxied site, generated from `bunkerweb_services` in `host_vars/bunkerweb101.yml`), restarting only if something actually changed — see [Configuring BunkerWeb](#configuring-bunkerweb) |
...
4. Register with Katello (`katello_register_host` role) via `hammer host-registration generate-command`, then confirms Katello registered it under the exact hostname (`hammer host info --name`, failing loudly on any mismatch — this is what lets `decommission-vm.yml` find it by `new_vm_name` alone, no guessing later), disable every enabled repo that isn't Katello-managed — determined dynamically (diff the repo IDs in `/etc/yum.repos.d/redhat.repo`, which the registration script writes, against `dnf repolist all`'s enabled set) rather than a fixed list, since which default vendor repos ship enabled varies by template/version (e.g. `epel` isn't always present). Left-enabled vendor repos conflict with Katello's own content-view repos on tightly version-locked packages like `selinux-policy-targeted`/`-extra` — a real incident on `patchmon101` where `vm-updates.yml` failed with a depsolve error. Fails clearly instead of disabling everything if `redhat.repo` turns out empty (registration not having worked as expected). Then applies OS updates, installs `podman`+`git`.
...
`decommission-vm.yml` (`-e new_vm_name=<hostname>`, optionally `-e decom_skip_backup=true` to skip step 2 entirely — e.g. for a short-lived test VM that was never worth backing up) is the reverse of `provision-vm.yml` — no confirmation flag by design (just the name), but every step is set up to fail loudly rather than guess:
...
2. Migrate to `vm_backup_node` if it isn't already there, then take a final `vzdump` safety backup to pCloud (`pve_migrate_vm` + `vzdump_backup`, the exact same roles/pattern `vm-backups.yml` uses) — a last-resort rollback if the decommission turns out to be a mistake. Skipped entirely (migration included, since there's no point moving it for a backup that won't happen) when `decom_skip_backup` is set.
...
`bunkerweb101` (WAF/reverse-proxy, `bunkerweb-all-in-one` image) is configured entirely by `configure-bunkerweb.yml` (`bunkerweb_configure` role) from `bunkerweb_services` in `host_vars/bunkerweb101.yml` — a list of proxied sites (name, domain, and any settings beyond the `USE_REVERSE_PROXY=yes` baseline). Each site's own `{{ domain }}_*` env vars are rendered into its own file under `/etc/bunkerweb/conf.d/<name>.env`; global settings (embedded services, `MULTISITE`, Let's Encrypt, DNS resolvers, LAN whitelist) go into `/etc/bunkerweb/global.env`, and `SERVER_NAME` is generated from the same `bunkerweb_services` list rather than hand-maintained. Adding or removing a proxied service is just editing that list and re-running the playbook — a removed entry's `conf.d/*.env` file is deleted automatically (found via `ansible.builtin.find`, diffed against the current list), so decommissioning a VM (e.g. `joplin101`) doesn't leave a stale reverse-proxy entry behind the way the original single-file config did.

Secrets (OVH DNS-01 API credentials, admin password — see [Secrets](#secrets)) are templated into a separate `/etc/bunkerweb/secrets.env` (`mode: '0600'`), never committed. The Podman Quadlet unit itself is templated too (not a static `copy:` like every other Quadlet in this repo), since its `EnvironmentFile=` lines are generated dynamically — one per service, plus `global.env`/`secrets.env`. `bunkerweb.service` only reloads/restarts if something actually changed (every templated file is `register`ed, ORed into one fact) — routine re-runs with no config drift are a no-op.
...
`BUNKERWEB_OVH_APPLICATION_KEY`/`BUNKERWEB_OVH_APPLICATION_SECRET`/`BUNKERWEB_OVH_CONSUMER_KEY` (OVH API credentials for Let's Encrypt's DNS-01 challenge) and `BUNKERWEB_ADMIN_PASSWORD` follow the same env-var-via-Variable-Group pattern, read via `bunkerweb_ovh_*`/`bunkerweb_admin_password` in `group_vars/all.yml`. Unlike every other secret above, `bunkerweb_configure` (used by `configure-bunkerweb.yml`) actually writes these to a file on the target host (`/etc/bunkerweb/secrets.env`, `mode: '0600'`, `no_log: true` on the templating task) — BunkerWeb's Podman Quadlet reads its config exclusively via `EnvironmentFile=`, there's no way to hand it a secret any other way. Missing these before the first run doesn't fail loudly: Let's Encrypt renewal and admin login just break at the next attempt.
